#!/usr/bin/env python3
"""Preset 17: Wave Interference - Multiple Wave Sources"""
...
void main() {
    ...
    // Kick adds wave sources
    int numWaves = 2 + int(iKickPulse * 6.0);
    ...
    for(int i = 0; i < 8; i++) {
        ...
        float angle = float(i) * 6.28 / float(numWaves) + iTime * 0.3 + iTom2Spin * 3.0;
        float radius = 0.4 + iTom1Morph * 0.3;
        vec2 source = vec2(cos(angle), sin(angle)) * radius;

        float dist = length(p - source);

        // Wave equation with kick frequency
        float freq = 10.0 + iKickPulse * 20.0;
        float phase = iTime * 3.0;
        float wave = sin(dist * freq - phase) / (dist * 2.0 + 1.0);

        interference += wave;
    }

    // Hat creates random phase shifts
    if(iHatGlitch > 0.1) {
        float glitchPhase = fract(sin(dot(floor(p * 10.0), vec2(12.9898, 78.233))) * 43758.5453);
        interference += sin(glitchPhase * 6.28 + iTime * 10.0) * iHatGlitch * 0.5;
    }

    // Visualize interference pattern
    float pattern = interference / float(numWaves);
    pattern = pattern * 0.5 + 0.5;

    vec3 color = vec3(pattern);

    // Highlight constructive interference
    color += smoothstep(0.7, 1.0, pattern) * 0.3;
...
class ShaderVisualEngine:
    def __init__(self):
        ...
        pygame.display.set_caption('Preset 17: Wave Interference [9:16]')
