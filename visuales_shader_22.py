#!/usr/bin/env python3
"""Preset 22: Neural Noise - Multi-Octave Perlin Evolution"""
...
float neuralNoise(vec2 p, int octaves) {
    float value = 0.0;
    float amplitude = 0.5;
    float frequency = 1.0;
...
vec2 domainWarp(vec2 p, float amount) {
    float warpX = neuralNoise(p + vec2(0.0, 0.0), 4);
    float warpY = neuralNoise(p + vec2(5.2, 1.3), 4);
...
void main() {
    ...
    // Kick increases octave count
    int octaves = 4 + int(iKickPulse * 8.0);

    // Tom1 controls domain warping
    float warpAmount = iTom1Morph * 0.5;

    // Tom2 rotates the noise field
    float angle = iTom2Spin * 6.28 + iTime * 0.2;
    mat2 rot = mat2(cos(angle), -sin(angle), sin(angle), cos(angle));
    p = rot * p;

    // Apply domain warping
    p = domainWarp(p * 2.0, warpAmount);

    // Generate neural noise
    float n = neuralNoise(p + iTime * 0.1, octaves);

    // Hat creates turbulence layers
    if(iHatGlitch > 0.1) {
        vec2 turbP = p + vec2(sin(iTime), cos(iTime)) * iHatGlitch;
        float turb = neuralNoise(turbP * 4.0, 6);
        n = mix(n, turb, iHatGlitch * 0.6);
    }
    ...
    // Create bands
    float bands = sin(n * 20.0 + iTime) * 0.5 + 0.5;

    vec3 color = vec3(n);

    // Highlight features
    color += smoothstep(0.7, 0.9, n) * (0.3 + iKickPulse * 0.3);
...
class ShaderVisualEngine:
    def __init__(self):
        ...
        pygame.display.set_caption('Preset 22: Neural Noise [9:16]')
