#!/usr/bin/env python3
"""
Preset 11: Pixel Sorting - Digital Glitch Art
Category: Glitch/Feedback
Vertical and horizontal pixel sorting effects
"""
...
void main() {
    ...
    // Base pattern
    float pattern = noise(p * 5.0 + iTime * 0.2);

    // Pixel sorting effect
    vec2 sortUV = uv;

    // Hat triggers vertical sorting
    if(iHatGlitch > 0.1) {
        float column = floor(uv.x * iResolution.x / 4.0);
        float threshold = hash(vec2(column, floor(iTime * 5.0)));

        if(pattern > threshold - iHatGlitch * 0.5) {
            // Sort pixels vertically
            float offset = (hash(vec2(column, uv.y * 10.0)) - 0.5) * iHatGlitch;
            sortUV.y += offset;
        }
    }

    // Tom1 triggers horizontal sorting
    if(iTom1Morph > 0.1) {
        float row = floor(uv.y * iResolution.y / 4.0);
        float threshold = hash(vec2(floor(iTime * 5.0), row));

        if(pattern > threshold - iTom1Morph * 0.5) {
            float offset = (hash(vec2(uv.x * 10.0, row)) - 0.5) * iTom1Morph;
            sortUV.x += offset;
        }
    }

    // Resample pattern with sorted coordinates
    sortUV = fract(sortUV);
    vec2 sortP = (2.0 * sortUV * iResolution.xy - iResolution.xy) / iResolution.y;
    float sortedPattern = noise(sortP * 5.0 + iTime * 0.2);

    // Kick adds brightness bands
    float bands = sin(sortP.y * 20.0 + iTime * 3.0) * 0.5 + 0.5;
    sortedPattern += bands * iKickPulse * 0.5;

    // Tom2 rotation
    float angle = iTom2Spin * 6.28;
    float ca = cos(angle), sa = sin(angle);
    sortP = mat2(ca, -sa, sa, ca) * sortP;

    vec3 color = vec3(sortedPattern);

    // Kick flash
    color += iKickPulse * 0.3;

    // Glitch blocks
    float blockHash = hash(floor(sortP * 10.0 + iTime * 5.0));
    if(iHatGlitch > 0.3 && blockHash > 0.95) {
...
class ShaderVisualEngine:
    def __init__(self):
        ...
        pygame.display.set_caption('Preset 11: Pixel Sorting [9:16]')
