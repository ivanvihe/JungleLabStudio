...
// Función de ruido para efectos eléctricos
...
// Función para rayos eléctricos procedurales
fn lightningBolt(uv: vec2<f32>, start: vec2<f32>, end: vec2<f32>, time: f32, thickness: f32) -> f32 {
    ...
    // Proyectar UV sobre la línea del rayo
...
fn plasmaEffect(uv: vec2<f32>, time: f32, energy: f32) -> vec3<f32> {
    ...
    // Colores del plasma basados en energía
    let core_color = vec3<f32>(0.0, 1.0, 1.0);      // Cyan
    let mid_color = vec3<f32>(0.0, 0.5, 1.0);       // Azul
    let edge_color = vec3<f32>(0.5, 0.0, 1.0);      // Púrpura
    
    // Interpolación de colores basada en intensidad
...
fn fs_main(@builtin(position) frag_coord: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    // Color de fondo - azul oscuro energético
    ...
    // Rayos principales - múltiples rayos dinámicos
    ...
    // Color de los rayos - gradiente energético
    let lightning_color = mix(
        vec3<f32>(0.0, 0.8, 1.0),  // Azul eléctrico
        vec3<f32>(1.0, 0.0, 1.0),  // Magenta alta energía
        uniforms.audio_high
    );
    ...
    // Efectos de alta energía cuando el audio es intenso
    ...
    // Pulsación global sincronizada con audio
