...
// Función para generar ruido procedural
fn noise(p: vec2<f32>) -> f32 {
    let K1 = vec2<f32>(23.14069263277926, 2.665144142690225);
    return fract(cos(dot(p, K1)) * 12345.6789);
}

// Función para ruido suavizado
...
// Función para líneas procedurales dinámicas
...
// Función para crear patrones de líneas complejos
fn createAbstractPattern(uv: vec2<f32>, time: f32, audioIntensity: f32) -> f32 {
    var pattern = 0.0;
    
    // Líneas primarias (controladas por bajos)
    ...
    // Líneas secundarias (controladas por medios)
    ...
    // Líneas de detalle (controladas por agudos)
...
// Función para crear ondulaciones de fondo
...
fn fs_main(@builtin(position) frag_coord: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    // Mezclar colores basado en la posición y tiempo para variedad
