#!/usr/bin/env python3
"""
Preset 8: Magnetic Field Lines - Force Field Visualization
...
// Magnetic field from multiple point charges
vec2 magneticField(vec2 p, vec2 charge, float strength) {
    vec2 diff = p - charge;
    float dist = length(diff) + 0.1;
    return diff / (dist * dist * dist) * strength;
}
...
void main() {
    ...
    // Kick adds more charges
    int numCharges = 2 + int(iKickPulse * 4.0);

    vec2 field = vec2(0.0);

    // Calculate total field from all charges
    for(int i = 0; i < 6; i++) {
        if(i >= numCharges) break;

        float angle = float(i) * 6.28 / float(numCharges) + iTime * 0.3 + iTom2Spin * 3.0;
        float radius = 0.5 + iTom1Morph * 0.4;
        vec2 chargePos = vec2(cos(angle), sin(angle)) * radius;

        float strength = (i % 2 == 0) ? 1.0 : -1.0;
        field += magneticField(p, chargePos, strength);
    }

    // Visualize field lines
    float fieldStrength = length(field);
    float lines = sin(atan(field.y, field.x) * 20.0 + length(p) * 10.0 - iTime * 2.0);

    vec3 color = vec3(0.0);

    // Field line intensity
    float intensity = smoothstep(0.8, 1.0, lines) * (0.3 + iKickPulse * 0.7);
    intensity *= exp(-fieldStrength * 0.5);

    color += intensity;

    // Hat glitch distorts field
    if(iHatGlitch > 0.1) {
        float glitch = hash(floor(p * 20.0 + iTime * 10.0));
        if(glitch > 0.9 - iHatGlitch) {
            color += vec3(0.5);
        }
    }
    ...
    for(int i = 0; i < 6; i++) {
        ...
        float dist = length(p - chargePos);
        color += (0.05 / (dist + 0.05)) * 0.3;
    }

    // Center glow
    color += exp(-length(p) * 1.5) * 0.2 * iKickPulse;
...
class ShaderVisualEngine:
    def __init__(self):
        ...
        pygame.display.set_caption('Preset 8: Magnetic Field [9:16]')
