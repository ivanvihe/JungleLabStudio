#!/usr/bin/env python3
"""Preset 18: Truchet Tiles - Procedural Pattern Evolution"""
...
float truchet(vec2 p) {
    ...
    // Choose tile rotation based on hash
    if(tile < 0.25) {
        return step(0.5, length(f - vec2(0.0, 0.0)));
    } else if(tile < 0.5) {
        return step(0.5, length(f - vec2(1.0, 0.0)));
    } else if(tile < 0.75) {
        return step(0.5, length(f - vec2(0.0, 1.0)));
    } else {
        return step(0.5, length(f - vec2(1.0, 1.0)));
    }
}
...
void main() {
    ...
    // Kick increases tile density
    float scale = 5.0 + iKickPulse * 15.0;

    // Tom1 animates tiles
    float animTime = iTime * (0.5 + iTom1Morph * 2.0);
    ...
    // Animated offset
    p += vec2(sin(animTime * 0.5), cos(animTime * 0.5)) * 0.3;

    float pattern = truchet(p * scale);

    // Hat glitch shuffles tiles
    if(iHatGlitch > 0.1) {
        vec2 glitchOffset = vec2(hash(floor(p * scale) + iTime * 10.0), hash(floor(p * scale) + iTime * 10.0 + 100.0));
        glitchOffset = (glitchOffset - 0.5) * iHatGlitch;
        float glitchPattern = truchet((p + glitchOffset) * scale);
        pattern = mix(pattern, glitchPattern, iHatGlitch * 0.5);
    }

    vec3 color = vec3(pattern);

    // Add glow to pattern
    color += (1.0 - pattern) * 0.2 * iKickPulse;
...
class ShaderVisualEngine:
    def __init__(self):
        ...
        pygame.display.set_caption('Preset 18: Truchet Tiles [9:16]')
