#!/usr/bin/env python3
"""
Preset 6: Sierpinski Pyramid - Tetrahedral Fractal
Category: Fractal/SDF Advanced
3D triangular fractal evolution
"""
...
// Tetrahedron folding for Sierpinski
...
float map(vec3 p) {
    int iterations = 3 + int(iKickPulse * 5.0);
    float scale = 2.0 + iTom1Morph;

    if(iHatGlitch > 0.1) {
        p += vec3(hash(p.xy + iTime), hash(p.yz + iTime), hash(p.xz + iTime) - 0.5) * iHatGlitch * 0.3;
    }
    ...
    return sdTetrahedron(p, 1.0) * pow(scale, -float(iterations));
}
...
void main()
{
    ...
    float angle = iTime * 0.4 + iTom2Spin * 3.0;
    vec3 ro = vec3(3.5 * cos(angle), 1.0, 3.5 * sin(angle));
    vec3 ta = vec3(0.0);
    vec3 fw = normalize(ta - ro);
    vec3 rt = normalize(cross(vec3(0.0, 1.0, 0.0), fw));
    vec3 up = cross(fw, rt);
    vec3 rd = normalize(fw + p.x * rt + p.y * up);
    ...
    if(t < 20.0) {
        ...
        vec3 lig = normalize(vec3(2.0, 3.0, 4.0));

        float diff = max(dot(nor, lig), 0.0);
        float spec = pow(max(dot(reflect(-lig, nor), -rd), 0.0), 16.0);

        color = vec3(0.6 + iKickPulse * 0.4) * diff + spec * 0.3;
        color *= 1.0 - t / 20.0;
    }

    color += exp(-length(p) * 1.2) * 0.3 * iKickPulse;
    color *= 1.0 - 0.4 * length(p * 0.5);
...
class ShaderVisualEngine:
    def __init__(self):
        ...
        pygame.display.set_caption('Preset 6: Sierpinski Pyramid [9:16]')
